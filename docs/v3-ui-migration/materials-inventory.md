...
- **Chapter 3 ✅ (commit ถัดไป) — materials ครบทั้งหน้าแล้ว** — ประวัติธุรกรรม: filter (วันที่ preset/custom + ประเภท รับ/เบิก/นับ + กรองวัสดุ) + list จัดกลุ่มตามวัน + pagination โหลดเพิ่ม + detail modal + ลิงก์ดูสต๊อก · lazy-load ตอนแตะแท็บครั้งแรก · verified mobile: 7วัน=16 รายการ/30วัน=41 จัดกลุ่ม 14 วัน, filter เบิก=27 ล้วน, detail 6 แถว+hero+ลิงก์, 0 error (net disconnect ชั่วคราวตอนเทสต์ = catch จัดการถูก ไม่ใช่บั๊ก) · ต้นทางเดิม: `loadMatHistory` L2804/`fetchMatHistory`/`renderMatHistory` L2959/`renderMatHistTx`/`loadMoreMatHist`/`openMatHistDetail` + filter (`setMatHistRange`/`setMatHistType`/date custom `openMatHistDateModal`/`applyMatHistCustomDate`/mat picker `openMatHistMatPicker`) + helper `mh*` (date/format L3292-3392) · `matHistState`
